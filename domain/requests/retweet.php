...
if (isset($_POST['qoute']) && !empty($_POST['qoute'])) {
    ...
    else $for_user = PostDBConnector::getTweet($tweet_id)->user_id;
    ...
    if ($flag == false && $qoq == false && PostDBConnector::isRetweet($tweet_id)) {
        $flag_retweeted = PostDBConnector::userRetweeetedIt($user_id, $retweet->tweet_id);
    } else $flag_retweeted = PostDBConnector::userRetweeetedIt($user_id, $tweet_id);
...
if (isset($_POST['retweet']) && !empty($_POST['retweet'])) {
    ...
    if ($flag) {
        ...
        if (PostDBConnector::isRetweet($tweet_id) && $retweet->retweet_msg == null) {
            $data_tweet = [
                'post_id' => $post_id,
                'retweet_msg' => null,
                'retweet_id' => $retweet->post_id,
...
if (isset($_POST['unretweet']) && !empty($_POST['unretweet'])) {
    ...
    if (isset($retweet->tweet_id)) {
        $for_user = PostDBConnector::getTweet($retweet->tweet_id)->user_id;
        $target = $retweet->tweet_id;
    } else {
        $for_user = PostDBConnector::getRetweet($retweet->retweet_id)->user_id;
...
if (isset($_POST['option']) && !empty($_POST['option'])) {
    ...
    if ($retweet_sign && $user_retweeted_it) {
        $retweeted_user = PostDBConnector::getRetweet($tweet_id);
    ...
    <div class="retweet-div">
        <a href="#"
           ...
           if (($user_retweeted_it && !$retweet_sign) || $flaga) {
               if ($flaga == false)
                   echo PostDBConnector::retweetRealId($tweet_id, $user_id);
               else {
                   if ($retweeted_user->tweet_id != null)
                       echo PostDBConnector::retweetRealId($retweeted_user->tweet_id, $user_id);
                   else echo PostDBConnector::retweetRealId($retweeted_user->retweet_id, $user_id);
               }
           } else echo $tweet_id; ?>"
            ...
            <div class="retweet-popup-footer">
                <div class="retweet-popup-footer-right">
                    <button class="qoute-it"
                            ...
                            data-qoq="<?php echo $qoq; ?>"
                            type="submit"><i class="fas fa-pencil-alt" aria-hidden="true"></i>Qoute
                    </button>
                </div>
            </div>
        </div>
